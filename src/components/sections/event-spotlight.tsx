...
export function EventSpotlight({ showHeading = true }: { showHeading?: boolean }) {
  ...
  return (
    <section className="mx-auto max-w-site px-5 pb-28">
      <Reveal className="relative overflow-hidden rounded-[32px] border border-mint-ink/22 bg-[radial-gradient(900px_500px_at_20%_0%,--alpha(var(--color-glow)/16%),transparent_60%),radial-gradient(700px_500px_at_100%_100%,--alpha(var(--color-glow)/8%),transparent_60%),var(--color-spotlight)] px-7 py-16 lg:px-16 lg:py-20">
        <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_1fr]">
          <div>
            ...
            <p className="mt-8 bg-gradient-to-b from-tint to-headline-end bg-clip-text text-[clamp(64px,10vw,132px)] leading-[0.9] font-semibold tracking-[-0.06em] text-transparent">
              <CountUp to={10000} />+
            </p>
            ...
            <ul className="mt-7 grid grid-cols-2 gap-2.5">
              {lane.services.slice(0, 4).map((s) => (
                <li key={s.name} className="rounded-2xl border border-glass-line bg-tint/3 px-4 py-3.5">
